  ...
  <form @submit.prevent="submitForm">
    ...
    <div class="customer-address-payment-form">
      <div class="address-form-wrapper">
        ...
        <div v-if="show_default" class="row">
          <div class="col-12">
            <div class="form-group">
    ...
    <div class="field">
      <p class="control">
        <button class="btn btn-fill-out btn-sm">Update</button>
        <nuxt-link to="/customer/address" class="btn btn-dark btn-sm">
          Cancel
        </nuxt-link>
      </p>
    </div>
...
export default {
  ...
  async asyncData({ $axios, params, app }) {
    const address = await $axios.$get(`addresses/${params.id}`);
    return {
      form: address.data,
    };
  },
  ...
  watch: {
    'form.is_default'(val) {
      this.form.is_default = val === 1 || val === true ? 1 : 0;
    },
    'form.country_id'(val) {
    ...
    async submitForm() {
      ...
      if (!this.$v.$invalid) {
        try {
          const self = this;
          await this.$axios
            .$put(`customer/addresses/${this.form.id}`, this.form)
